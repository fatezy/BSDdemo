...
public class TransferClient {
    ...
    private String sendFilePath = FileConstants.SEND_FILE_PATH;

    private static ExecutorService executorService = Executors.newCachedThreadPool();

    /**
     * 带参数的构造器，用户设定需要传送文件的文件夹s
     *
     * @param filePath
     */
...
//        ExecutorService executorService = Executors.newCachedThreadPool();
     ...
     * @param filePath
     */
    public void singleFileService(String filePath) {
        executorService.execute(sendFile(filePath));
    ...
    private static Runnable sendFile(final String filePath) {
        return new Runnable() {
            ...
            private int port = FileConstants.DEFAULT_BIND_PORT;
...
//        new TransferClient().singleFileService("G:\\send\\message.txt");
//        new TransferClient().singleFileService("G:\\send\\evn.txt");
//        new TransferClient().singleFileService("G:\\send\\a.py");
        File file = new File("G:\\send\\123\\message.txt");
